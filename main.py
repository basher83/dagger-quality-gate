...
class QualityGatePipeline:
    ...
    async def run(self) -> int:
        ...
        async with dagger.Connection() as client:
            ...
            if not enabled_checks:
                console.print("[yellow]⚠️  No checks enabled. Set ENABLE_<CHECK>=true to enable checks.[/yellow]")
                return 0
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                final_results.append(
                    CheckResult(check_name, False, error=str(result))
                )
            else:
    ...
    async def _run_check(
        ...
        try:
            ...
            elif check_name in ["ruff", "mypy", "ty"]:
            ...
            return CheckResult(
                check_name,
                False,
                error=f"Error running check: {str(e)}"
            )
